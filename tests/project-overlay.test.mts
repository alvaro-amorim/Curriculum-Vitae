...
test("published MongoDB content overrides a static project", () => {
...
test("invalid published content preserves a valid static fallback", () => {
  const original = projects[0];
  const result = applyPublicProjectOverlay(projects, [
    {
      content: {
...
test("project detail returns null for an explicit draft override", () => {
  const original = projects[0];
