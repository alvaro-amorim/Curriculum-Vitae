...
const LocalizedListSchema = z.object({
  en: z.array(ShortTextSchema).max(24),
  pt: z.array(ShortTextSchema).max(24),
}).strict();
...
export const ProjectContentSchema = z.object({
  ...
  visuals: z.object({
    ...
    gallery: z.array(z.string().trim().min(1).max(2_048)).max(12),
    heroImage: z.string().trim().min(1).max(2_048).nullable(),
    layout: z.enum([
      "operational-saas",
      "social-ai",
      "crm-pipeline",
    ...
    thumbnail: z.string().trim().min(1).max(2_048).nullable(),
  }).strict().optional(),
...
export const AdminProjectMutationSchema = z.object({
  ...
  if (value.publicationStatus === "published" && !value.project.links.website) {
    context.addIssue({
      code: "custom",
      message: "Projetos publicados precisam de um link de website.",
      path: ["project", "links", "website"],
    });
  }
});
