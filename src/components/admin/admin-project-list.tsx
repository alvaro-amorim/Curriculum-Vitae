...
export function AdminProjectList({
  ...
  async function importCatalog() {
    ...
    try {
      ...
      setTone("success");
      setMessage(`${body.data?.imported ?? 0} projetos importados ou atualizados.`);
      router.refresh();
    } catch {
      setTone("error");
    ...
    <>
      {!databaseReady ? (
        <div className={styles.notice} data-tone="error">
          <span>A migration do Admin CRUD ainda não foi aplicada no Supabase. O portfólio público continua usando o fallback estático.</span>
        </div>
      ) : null}
      ...
      <div className={styles.toolbar}>
        <span>{projects.length} registros administrativos</span>
        <div className={styles.headerActions}>
          <button
            ...
            type="button"
          >
            {importing ? "Importando..." : "Importar catálogo atual"}
          </button>
      ...
      {projects.length > 0 ? (
        ...
        <div className={styles.emptyState}>
          <p>Não há registros no banco. Aplique a migration e importe o catálogo versionado para começar.</p>
        </div>
      )}
    </>
  );
